...
class MatrixEigenspaceProjector(LowRankBasisProjector):
    """
    Проектор на top-k собственное пространство симметричной матрицы.

    Матрица задаётся явно или через мат-век произведение.

    Аргументы:
    params - параметры модели
    k - размерность подпространства
    matrix - опционально явно заданная симметричная матрица размера (num_params, num_params)
    matvec - опционально Callable v -> Av, матрично-векторное произведение
    solver:
        "eigsh" - Sci-Py ARPACK над LinearOperator
        "dense_eigh" - для явно заданных плотных симметричных матриц
        "cola_lanczos" - для cola-ml алгоритма Ланцоша
    which:
        для solver="eigsh"
        - "LA" - наибольшие алгебраически собственные значения
        - "LM" - наибольшие по магнитуде собственные значения
    """
