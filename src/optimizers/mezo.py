...
class MeZO(Optimizer):
    """Zeroth-order SGD с опциональной проекцией на подпространство.

    Args:
        params - итерируемый объект с параметрами или группами параметров
        lr - щаг обучения
        eps - масштаб возмущения.
        weight_decay - коэффициент decoupled L2-регуляризации
        seed - необязательный seed для отдельного torch.Generator
        subspace_sampling - если True и в момент вызова step передан проектор
        с уже подготовленным базисом, направление сэмплируется напрямую внутри
        подпространства проектора: z = Q @ xi, где xi \sim N(0, I_k). По умолчанию False.
    """
